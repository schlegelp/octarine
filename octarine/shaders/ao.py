...
# Reading the depth buffer. Used by the occlusion pass and the blur, both of
# which bind `depthTex` and carry a `projection_transform_inv` uniform.
_DEPTH_WGSL = """
...
# Normals are reconstructed from the depth buffer rather than rendered into
# a G-buffer of their own: pygfx hands effect passes the color and depth
